const { UserModel, SeedModel, ReviewModel } = require('../models')
...
const upload = multer({
  storage: multerS3({
    ...
    key: function (req, file, cb) {
      ...
      cb(
        null,
        Date.now().toString() + file.originalname + Date.now().toString(),
      )
    },
  }),
  preservePath: true,
...
const storage = new CloudinaryStorage({
  ...
  params: {
    ...
    format: async (req, file) =>
      file.originalname.substr(file.originalname.lastIndexOf('.') + 1), // supports promises as well
...
const actions = {
  ...
  writeReview: asyncMiddleware(async (req, res) => {
    let { id } = req.decoded
    ...
    let rate = await ReviewModel.create({ ...req.body, user: id })
    if (!rate) {
      return res.status(statusCodes.client.badRequest).json({
...
router.post('/write-review', jwt.verifyJwt, actions.writeReview)
